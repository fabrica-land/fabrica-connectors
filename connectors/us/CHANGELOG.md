...
- **Short-form defined terms (Section 2.6):** Introduced "Smart Wallet" (Section 7.3(a) Contract Account — multisig, account-abstraction wallets, and other custody solutions directly controlled by the Beneficiary) and "Functional Contract" (Section 7.3(b) Contract Account — escrow, bridges, lending protocols, collateral management) as short-form terms defined within the Contract Account definition. Replaced all verbose "Contract Account of the type described in Section 7.3(a/b)" references throughout with the short forms
- **Section 7.3 updated:** Sub-clauses (a) and (b) now explicitly name the short-form terms (Smart Wallet, Functional Contract) with expanded examples including account-abstraction wallets and lending protocols
- **Section 7.5 restructured:** Split from a single long paragraph into four numbered sub-provisions: (1) unauthorized transfers do not transfer beneficial ownership, (2) Beneficiary security responsibility, (3) qualifying purchaser/protected secured party carve-out, (4) conflict resolution
- **Readability improvements:** Broke long compound sentences into shorter statements throughout Sections 10.2–10.4. Converted embedded lists in Sections 10.2.3, 10.2.4, and 10.2.5 to bullet-point format. Replaced passive "In the event" constructions with direct "If" clauses. Fixed "off-chain" to "offchain" for style consistency
- **Terminology consistency (Account or Smart Wallet):** All references to the holder of the Property Token now consistently use "Account or Smart Wallet" instead of bare "Account" where the context also applies to Smart Wallets. Affected Sections 1.3, 7.4, 7.5, 10.2.3, 10.2.4, 10.2.5, 10.3.3, 10.3.4. Section 7.5.1 also expanded "private keys" to "private keys or signing credentials" to cover multisig and account-abstraction wallet compromise scenarios
- **Section 10.2.3 — Smart Wallet DoS prevention:** Narrowed the "originating from" definition for Smart Wallets so that only transactions in which the Smart Wallet's own execution logic initiates an outbound call or state change qualify as halt triggers. Transactions sent *to* the Smart Wallet by external parties — including calls that revert or produce no outbound action — do not halt the Quiet Period. This closes a potential denial-of-service vector where a third party could repeatedly poke a Smart Wallet to block recovery
- **Contest/certificate/deed recital alignment:** Section 10.2.4(b) and 10.2.5(c) now reference "contest as described in Section 10.2.2" instead of only listing counter-notices and lis pendens, ensuring competing Notices of Lost Token Control are tracked consistently through the certificate and deed recital language. Removed undefined "counter-claim" from 10.2.5(c)
- **Section 4.1 — applicable law limiter:** Added "to the maximum extent permitted by applicable law" to the digital-signature-equivalence provision, preserving the core thesis while acknowledging that some jurisdictions may impose additional formality requirements (e.g., notarization)
- **Section 8.1.3 — Trustee liability standard:** Changed "willful actions" to "willful misconduct or gross negligence" to match standard trustee exculpation language and avoid unintended liability for good-faith acts
- **Section 10.2.2 — constructive notice precision:** Changed "constructive notice to the world" to "constructive notice under applicable recording statutes" for jurisdictional accuracy
- **Sections 10.2 and 10.3 — trigger condition consistency:** The opening sentences of Sections 10.2 and 10.3 now explicitly reference "Account or Smart Wallet" and "private key(s) or signing credentials" to match the eligibility rules and avoid ambiguity when the token is held in a Smart Wallet
- **Section 10.2.3 — "originating from" tightened:** Changed "initiates an outbound call or state change" to "successfully executes an outbound action" to eliminate ambiguity about failed or state-only operations and clarify that only successful outbound actions by the Smart Wallet halt the Quiet Period
- **Section 6.3 — Fractionalization governance cross-reference:** Added new provision stating that if the Property Token is fractionalized, the governance of fractional interests (decision thresholds, deadlock, partition) is governed by a separate fractionalization agreement. This Agreement applies to fractional owners as a group but does not prescribe internal governance
- **Section 7.1 — Trustee default rule:** Added provision that unless the Beneficiary has appointed a different Trustee by separate written instrument, the Beneficiary is deemed to be the Trustee. This closes the gap where a new Beneficiary might not formally appoint a trustee
- **Section 3.3 removed (vestigial provision):** Deleted "If the Property Token is minted by a Contract Account, the Grantor shall be considered the Beneficiary of the Trust, unless otherwise specified." This provision was vestigial — the mint() function takes a recipients array, so whoever receives the token is the owner regardless of what contract or EOA submits the minting transaction. The remaining Section 3.3 (previously 3.4) handles the only relevant edge case: token not yet minted
- **Section 3.1.3 — Conveyance deed attachment clarified:** The Agreement is attached via the `operatingAgreement` field. A copy of the conveyance deed may be attached to facilitate validation but is not required for the Trust to be legally effective. The deed lives in the mutable `configuration` object (not immutable `definition`), reflecting its role as a validation artifact rather than a structural requirement
